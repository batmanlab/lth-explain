...
def calculate_TCAV_score_multi_class_classification_derma(
        bb_model,
        bb_model_bottom,
        bb_model_top,
        data_loader,
    ...
    sample_size = 0
    bb_model.eval()
    bb_model_bottom.eval()
    bb_model_top.eval()
    for x, y in tcav_bar:
        x = x.to(device)
        if class_label_index == y.item():
            sample_size += 1
            activation = bb_model_bottom(x)
            output_prob1, activation1 = bb_model_top(activation)
            logit1 = output_prob1[:, class_label_index]
            grads = torch.autograd.grad(logit1, activation1, retain_graph=True)[0].cpu().detach().numpy()
            flattened_grad = np.squeeze(grads)
            if np.dot(flattened_grad, cavs) > 0:
                pos_grad += 1
...
def get_flattened_grads(cav_flattening_type, grads):
    if cav_flattening_type == "max_pooled":
        ...
        return utils.flatten_cnn_activations_using_adaptive_avg_pooled(
            grads,
            kernel_size=grads.shape[-1],
            stride=1
        )
...
def calculate_cavs_multiclass(
    ...
    device = utils.get_device()
    class_to_compute = class_labels[labels_index_TCAV]
    TCAV_arr = []
    print(
        f"########## TCAV for model: {model_arch} || layer: {bb_layer} || class: {class_to_compute} #############"
    )
    for concept_id in concept_index_list_TCAV:
        concept_name = concept_names[concept_id]
        cavs = torch_concept_vector[concept_id].numpy()
        cavs /= np.linalg.norm(cavs)
    ...
    stat = {
        ...
        "class_to_predict_id": labels_index_TCAV,
        "concept_arr": TCAV_arr
    }
    return stat


def calculate_cavs_multiclass_derma(
        test_loader,
        torch_concept_vector,
        model,
        model_bottom,
    ...
    for concept_id in concept_index_list_TCAV:
        concept_name = concept_names[concept_id]
        cavs = torch_concept_vector[concept_id].cpu().numpy()
        cavs /= np.linalg.norm(cavs)
        print(f"========>> Concept: {concept_name} <<=======")
        print(f"Shape of cav: {cavs.shape}")
        print(f"Norm of cav: {np.linalg.norm(cavs)}")

        TCAV_score = calculate_TCAV_score_multi_class_classification_derma(
            model,
            model_bottom,
            model_top,
            test_loader,
