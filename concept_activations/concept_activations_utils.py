...
def get_normalized_vc(
        ...
        val_after_th,
        cav_flattening_type):
    if cav_flattening_type == "max_pooled" or cav_flattening_type == "avg_pooled" or \
            cav_flattening_type == "adaptive_avg_pooled":
...
def get_concept_to_pred(
    ...
    concept_to_pred = bb_model_tail(avgpool(concept_to_act).reshape(-1, activations.size(1) * 1 * 1))
    return concept_to_pred

# g_model input size: 21168
# g_model output size: 401408
...
def get_g_model_ip_op_size(
    ...
    if dataset_name == "mnist":
        ...
        activations, np_y_hat_bb, np_y_gt = next(iter(loader))
    if cav_flattening_type == "max_pooled" or cav_flattening_type == "avg_pooled" or cav_flattening_type == "adaptive_avg_pooled":
        g_model_ip_size = 14 * 14 * torch_concept_vector.size(0)
        g_model_op_size = 14 * 14 * torch_concept_vector.size(1)
...
def get_concept_vectors_for_pruning(_ite, cav_path, bb_layer, cav_flattening_type):
    ...
    if cav_flattening_type == "max_pooled":
        ...
        cav_vector_file = f"adaptive_avg_pooled_train_cavs_prune_iteration_{_ite}.pkl"

    cav_file = open(os.path.join(cav_path, cav_vector_file), "rb")
